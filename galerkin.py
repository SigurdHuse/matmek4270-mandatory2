...
class NeumannLegendre(Composite, Legendre):
    ...
    def basis_function(self, j, sympy=False):
        if sympy:
            return sp.legendre(j, x) - j * (j + 1) / ((j + 2) * (j + 3)) * sp.legendre(
                j + 2, x
            )
...
class NeumannChebyshev(Composite, Chebyshev):
    ...
    def basis_function(self, j, sympy=False):
        beta = lambda i: -(i**2) / (i**2 + 4 * i + 4)
        if sympy:
            return sp.cos(j * sp.acos(x)) + beta(j) * sp.cos((j + 2) * sp.acos(x))
        return Cheb.basis(j) + beta(j) * Cheb.basis(j + 2)
...
def test_project():
    ...
    domain = (0, 10)
    #
    for space in (Legendre, Chebyshev):
